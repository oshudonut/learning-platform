...
async function HomeContent() {
            ...
            <Link key={doc.id} href={`/document/${doc.id}`}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{formatDistanceToNow(doc.createdAt)}</p>
                  <div className="flex items-center gap-2 mt-2">
                    {doc.hasReviewer && (
                      <span className="text-[10px] bg-foreground/8 text-foreground/70 px-2 py-0.5 rounded-full">Reviewed</span>
                    )}
                    {doc.hasQuiz && (
                      <span className="text-[10px] bg-success/10 text-success px-2 py-0.5 rounded-full">Quiz ready</span>
                    )}
                  </div>
                </div>
              </div>
  ...
  // New user or logged-out — show landing
  return (
    <>
      {/* Hero */}
